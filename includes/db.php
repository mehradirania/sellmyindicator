...
/* SQLite is stored outside the public web root. */
...
function cd_migrate(PDO $pdo): void
{
    ...
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_orders_product ON orders(product_id)');
}

function cd_seed(PDO $pdo): void
{
    $stmt = $pdo->prepare('INSERT INTO products
        (id, name, description, price, image, file, wallet, created_at)
    ...
    $stmt->execute([
        ...
        ':price' => 299.0,
        ':image' => 'IMG_20260621_174250_857.jpg',
...
{
    return cd_db()->query('SELECT * FROM products ORDER BY created_at DESC')->fetchAll();
}

function cd_product(string $id): ?array
{
    $stmt = cd_db()->prepare('SELECT * FROM products WHERE id = :id LIMIT 1');
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
    return $row ?: null;
}
